...
class PEQFilter(ABC):
    ...
    def fr(self):
        ...
        self._fr = 10 * np.log10(
            (b0 + b1 + b2) ** 2 + (b0 * b2 * phi -
                                   (b1 * (b0 + b2) + 4 * b0 * b2)) * phi
        ) - 10 * np.log10(
            (a0 + a1 + a2) ** 2 + (a0 * a2 * phi -
                                   (a1 * (a0 + a2) + 4 * a0 * a2)) * phi
        )
        return self._fr
...
class Peaking(PEQFilter):
    ...
    def init(self, target):
        ...
        with warnings.catch_warnings():
            ...
            positive_peak_ixs, peak_props = find_peaks(
                np.clip(target, 0, None), width=0, prominence=0, height=0)
            negative_peak_ixs, dip_props = find_peaks(
                np.clip(-target, 0, None), width=0, prominence=0, height=0)
        ...
        widths = np.concatenate(
            [peak_props['widths'], dip_props['widths']])[mask]
        heights = np.concatenate(
            [peak_props['peak_heights'], dip_props['peak_heights']])[mask]
        # Find the biggest peak, by height AND width
        sizes = widths * heights  # Size of each peak for ranking
        # Index to indexes array which point to the biggest peak
        ixs_ix = np.argmax(sizes)
        ix = peak_ixs[ixs_ix]  # Index to f and target

        params = []
        if self.optimize_fc:
            self.fc = np.clip(self.f[ix], self.min_fc, self.max_fc)
            # Convert to logarithmic scale for optimizer
            params.append(np.log10(self.fc))
        if self.optimize_q:
            width = widths[ixs_ix]
...
class HighShelf(ShelfFilter):
    def init(self, target):
        ...
        if self.optimize_fc:
            ...
            ix = np.argmax([np.abs(np.mean(target[ix:]))
        ...
        if self.optimize_gain:
            ...
            # weight vector
            self.gain = 1
            self.gain = np.dot(target, self.fr) / \
                np.sum(self.fr)  # Weighted average
            self.gain = np.clip(self.gain, self.min_gain, self.max_gain)
            params.append(self.gain)
        return params
...
class LowShelf(ShelfFilter):
    def init(self, target):
        ...
        if self.optimize_fc:
            ...
            min_ix = np.sum(self.f < max(40, self.min_fc))
            max_ix = np.sum(self.f < min(10000, self.max_fc))
            ix = np.argmax([np.abs(np.mean(target[:ix + 1]))
                           for ix in range(min_ix, max_ix)])
            ix += min_ix
            self.fc = np.clip(self.f[ix], self.min_fc, self.max_fc)
            params.append(np.log10(self.fc))
...
class PEQ:
    ...
    def from_dict(cls, config, f, fs, target=None):
        ...
        filter_classes = {'LOW_SHELF': LowShelf,
                          'PEAKING': Peaking, 'HIGH_SHELF': HighShelf}
        keys = ['fc', 'q', 'gain', 'min_fc', 'max_fc',
                'min_q', 'max_q', 'min_gain', 'max_gain', 'type']
        ...
        global_filter_defaults['HIGH_SHELF'] = deepcopy(
            global_filter_defaults['LOW_SHELF'])
    ...
    def add_filter(self, filt):
        if filt.fs != self.fs:
            raise ValueError(
                f'Filter sampling rate ({filt.fs}) must match equalizer sampling rate ({self.fs})')
        if not np.array_equal(filt.f, self.f):
            raise ValueError(
                'Filter frequency array (f) must match equalizer frequency array')
    ...
    def sort_filters(self):
        ...
        self.filters = sorted(
            self.filters,
            key=lambda filt: type_order.index(
                filt.__class__.__name__) + filt.fc / 1e6,
            reverse=False)
    ...
    def markdown_table(self):
        ...
        table_data = [
            [i + 1, filt.__class__.__name__,
                f'{filt.fc:.0f}', f'{filt.q:.2f}', f'{filt.gain:.1f}']
    ...
    def to_dict(self):
        """PEQ as dictionary"""
        name_map = {LowShelf.__name__: 'LOW_SHELF',
                    Peaking.__name__: 'PEAKING', HighShelf.__name__: 'HIGH_SHELF'}
        return {
            'fs': self.fs,
    ...
    def _optimizer_loss(self, params, parse=True):
        ...
        loss_val = np.mean(np.square(
            target[self._min_f_ix:self._max_f_ix] - fr[self._min_f_ix:self._max_f_ix]))
    ...
    def _init_optimizer_params(self):
        ...
        order = [
            ...
            # Low shelfs with fixed fc and q
            [LowShelf.__name__, False, False],
            # High shelfs with fixed fc and q
            [HighShelf.__name__, False, False],
        ]

        def init_order(filter_ix):
            filt = self.filters[filter_ix]
            ix = order.index(
                [filt.__class__.__name__, filt.optimize_fc, filt.optimize_q])
            val = ix * 100
            if filt.optimize_fc:
        ...
        filter_argsort = sorted(
            list(range(len(self.filters))), key=init_order, reverse=True)
        remaining_target = self.target.copy()
        for ix in filter_argsort:  # Iterate sorted filter indexes
            filt = self.filters[ix]  # Get filter
            # Init filter and place params to list of lists
            filter_params[ix] = filt.init(remaining_target)
            remaining_target -= filt.fr  # Adjust target
        filter_params = np.concatenate(
            filter_params).flatten()  # Flatten params list
    ...
    def _callback(self, params, **args):
        ...
        moving_avg_loss = np.mean(
            np.array(self.history.loss[-n:])) if len(self.history.loss) >= n else 0.0
        ...
        if len(self.history.moving_avg_loss) > 1:
            ...
            change_rate = d_loss / \
                d_time if len(self.history.moving_avg_loss) > n else 0.0
        else:
            change_rate = 0.0
    ...
    def plot(self, fig=None, ax=None):
        ...
        if self.target is not None:
            ax.plot(self.f, self.target, color='black',
                    linestyle='--', linewidth=1, label='Target')
        for i, filt in enumerate(self.filters):
            ax.fill_between(filt.f, np.zeros(filt.fr.shape),
                            filt.fr, alpha=0.3, color=f'C{i}')
